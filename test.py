# Description: This script is used to test the model loading and inference process. It loads a model from a checkpoint file and performs inference on a sample image. The model is loaded from the "model_weights" directory and the model configuration is loaded from the "config" directory. The model is then used to perform inference on a sample image and the predicted action is displayed.
