...
sys.path.append("C:/Users/levid/Desktop/Uni/Master/SoSe24/ProjektLearningRobots/model/visualnav-transformer-main/train")
...
sys.path.append("C:/Users/levid/Desktop/Uni/Master/SoSe24/ProjektLearningRobots/model/visualnav-transformer-main/train/vint_train/diffusion_policy_main/diffusion_policy-main")
        ...
        for i, data in enumerate(tepoch):
            ...
            if i % print_log_freq == 0:
                losses = _compute_losses_nomad(
                            _ema_get_model(ema_model, model),
                            noise_scheduler,
                            batch_obs_images,
                            batch_goal_images,
                            distance.to(device),
            ...
            if image_log_freq != 0 and i % image_log_freq == 0:
                # 1) EMA updaten (returns None)
                ema_model.step(model.parameters())

                # 2) Modell fürs Visualisieren auswählen (EMA wenn vorhanden, sonst normales model)
                viz_model = _ema_get_model(ema_model, model)
                visualize_diffusion_action_distribution(
                    viz_model,#ema_model.step(model.parameters()),
                    noise_scheduler,
                    batch_obs_images,
                    batch_goal_images,
...
def _ema_get_model(ema_model, fallback_model):
    """
    Compatibility across diffusers versions.
    Returns the EMA model/module if available, else fallback_model.
    """
    if hasattr(ema_model, "averaged_model"):   # ältere API
        return ema_model.averaged_model
    if hasattr(ema_model, "model"):           # andere API
...
def evaluate_nomad(
    ...
    """
    goal_mask_prob = torch.clip(torch.tensor(goal_mask_prob), 0, 1)
    ema_net = getattr(ema_model, "averaged_model", None)
    if ema_net is None:
        ema_net = getattr(ema_model, "model", None)
    if ema_net is None:
        ema_net = model  # fallback to non-EMA model if EMA wrapper doesn't expose a module
