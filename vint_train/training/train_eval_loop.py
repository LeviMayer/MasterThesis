...
def train_eval_loop_nomad(
    ...
    """
    latest_path = os.path.join(project_folder, f"latest.pth")
    #ema_model = EMAModel(parameters=model.parameters(), model=model, power=0.75)#EMAModel(model=model,power=0.75)
    try:
    ...
    for epoch in range(current_epoch, current_epoch + epochs):
        ...
        ema_to_save = _ema_get_model(ema_model, model.module if isinstance(model, nn.DataParallel) else model)
        torch.save(ema_to_save.state_dict(), numbered_path)
