...
class ViNT_Dataset(Dataset):
    ...
    def __getitem__(self, i: int) -> Tuple[torch.Tensor]:
        ...
        if actions_np.dtype == object:
            # z.B. list of arrays -> stack to (T, D)
            actions_np = np.stack(
                [np.asarray(a, dtype=np.float32).squeeze() for a in actions_np],
                axis=0
            )
        else:
            actions_np = actions_np.astype(np.float32, copy=False)

        actions_torch = torch.from_numpy(actions_np)
        ...
        goal_pos_np = np.asarray(goal_pos)
        #neu
        if goal_pos_np.dtype == object:
            # z.B. [array([x,y])] oder [[x,y]] oder andere nested Varianten
            goal_pos_np = np.asarray(goal_pos_np.tolist(), dtype=np.float32).squeeze()
        else:
            goal_pos_np = goal_pos_np.astype(np.float32, copy=False)

        goal_pos_torch = torch.from_numpy(goal_pos_np)
