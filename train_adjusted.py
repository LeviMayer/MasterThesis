...
os.environ["OPENCV_VIDEOIO_PRIORITY_MSMF"] = "0"  # optional
import sys
import time
import yaml
import argparse
...
Wir gehen davon aus, dass diese Datei irgendwo im Repo liegt,
z.B. in vint_train/training/, und dass REPO_ROOT das Repo-Root ist.
"""
CURRENT_DIR = os.path.dirname(os.path.abspath(__file__))
REPO_ROOT = "/home/levi/ros2-turtlebot4-devcontainer/visualnav-transformer-main"#os.path.abspath(os.path.join(CURRENT_DIR, "..", ".."))  # ggf. anpassen
...
def main(config):
    ...
    if torch.cuda.is_available():
        ...
        print("Using cpu")
        config["gpu_ids"] = [0]  # Fallback, damit das Array existiert
    ...
    cudnn.benchmark = True  # gut, wenn Input-Sizes konstant sind
        ...
        for split in ["train", "test"]:
            if split in data_config:
                ...
                if split == "train":
                    ...
                    dataset_type = f"{dataset_name}_{split}"
                    test_dataloaders[dataset_type] = dataset

    # Alle Trainings-Datasets zusammenfassen
    ...
    # --- Multi-GPU ---
    if len(config["gpu_ids"]) > 1 and torch.cuda.is_available():
        model = nn.DataParallel(model, device_ids=config["gpu_ids"])
    model = model.to(device)

    # Optimizer / Scheduler nach DataParallel laden
...
if __name__ == "__main__":
    ...
    if config["use_wandb"]:
        ...
        wandb.init(
            ...
            #entity="gnmv2",  # ggf. anpassen
        )
        wandb.save(user_config_path, policy="now")
